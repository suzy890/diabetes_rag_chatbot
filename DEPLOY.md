...
## 관리자 대시보드 배포 (별도 앱 · PRD §8-1)

연구자용 대시보드(`src/admin_app.py`)는 **참여자 앱과 다른 URL로만** 접근한다. 같은 저장소에서 **메인 파일만 다르게** 지정해 **두 번째 앱**으로 배포한다.

1. share.streamlit.io → **"Create app"** (참여자 앱과 별개로 하나 더)
2. Repository·Branch 동일, **Main file path: `src/admin_app.py`**
3. **Secrets**: 참여자 앱과 같은 3개(SUPABASE_URL·SUPABASE_KEY·LLM_API_KEY)에 **관리자 비밀번호**를 추가
   ```toml
   SUPABASE_URL = "..."
   SUPABASE_KEY = "..."
   LLM_API_KEY  = "..."
   ADMIN_PASSWORD = "연구팀이 정한 비밀번호"
   ```
4. Deploy → 별도 URL 생성. 이 URL은 **참여자에게 공유하지 않는다.**

> 무료 플랜에서 완전 비공개(Private) 앱은 개수 제한이 있어, **공개 앱 + 비밀번호 게이트**(`ADMIN_PASSWORD`)로 연구자만 들어오게 한다.
